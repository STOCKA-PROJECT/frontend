...
const attributeValueByAttrId = computed(() => {
  const map = new Map<number, string | null>()
  for (const v of props.piece.attributeValues) map.set(v.attributeId, v.value)
  return map
})
...
function startEdit() {
  ...
  const attrs: Record<number, string | null> = {}
  for (const v of props.piece.attributeValues) attrs[v.attributeId] = v.value
  editAttributes.value = attrs
  editing.value = true
}
...
function submit() {
  ...
  for (const group of editAttributeGroups.value) {
    for (const attr of group.attributes) {
      const oldVal = attributeValueByAttrId.value.get(attr.id) ?? null
      const newRaw = editAttributes.value[attr.id]
      const newVal = newRaw == null || newRaw === '' ? null : newRaw
      if (newVal !== oldVal) {
        attrChanges.push({ attributeId: attr.id, value: newVal })
      }
    }
  }
...
function renderValue(attrId: number, type: PieceTypeAttributeResponseDto['type']): string {
  const v = attributeValueByAttrId.value.get(attrId)
  if (v == null || v === '') return '—'
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <dl v-if="!editing" class="info-grid">
      ...
      <div class="info-row">
        <dt>{{ t('dashboard.pieces.form.field_status') }}</dt>
        <dd>
          <span :class="['tag', piece.status === 'PENDING' ? 'tag-warn' : 'tag-ok']">
            {{ piece.status === 'PENDING' ? t('dashboard.pieces_table.status_pending') : t('dashboard.pieces_table.status_active') }}
          </span>
        </dd>
      </div>
      <div class="info-row">
        ...
        <section v-for="group in currentAttributeGroups" :key="group.typeId" class="attr-group">
          ...
          <dl class="info-grid">
            <div v-for="attr in group.attributes" :key="attr.id" class="info-row">
              ...
              <dd>{{ renderValue(attr.id, attr.type) }}</dd>
            </div>
          </dl>
        </section>
      </div>
