...
<template>
  ...
  <aside
    ...
    <DashboardOrgSwitcher v-if="hasOrgs" class="mt-4" />
    ...
    <nav class="flex flex-col gap-px" :aria-label="t('common.main_nav')">
      ...
      <NuxtLink v-if="hasOrgs" :to="orgPath('/timelines', navOrgSlug)" exact-active-class="is-active" class="nav-item">
    ...
    <div ref="userMenuRef" class="relative mt-auto">
      <button type="button"
        ...
        <div class="relative flex-shrink-0">
          ...
          <span v-if="hasPendingInvitations" class="notification-dot"
      ...
      <div v-if="menuOpen" role="menu"
        ...
        <NuxtLink :to="localePath('/dashboard/invitaciones')" role="menuitem"
          ...
          <span v-if="hasPendingInvitations"
            class="ml-auto inline-flex items-center justify-center rounded-full bg-accent px-1.5 text-[11px] font-semibold text-bg-card min-w-[18px] h-[18px]">
  ...
  <Teleport to="body">
    <Transition name="backdrop">
      <div v-if="isOpen" class="mobile-nav-backdrop fixed inset-0 z-40 bg-black/40 backdrop-blur-sm lg:hidden"
    ...
    <Transition name="drawer">
      <aside v-if="isOpen"
        ...
        role="dialog" aria-modal="true" :aria-label="t('common.main_nav')">
        <div class="flex items-center justify-end">
          <button type="button"
            ...
            :aria-label="t('common.close_menu')" @click="close">
        ...
        <div class="mt-auto">
          <button type="button"
            ...
            <div class="flex min-w-0 flex-1 flex-col">
              <span class="truncate text-[13px] font-medium text-ink">{{ userFullName || t('common.user_fallback')
                }}</span>
              <span class="truncate text-[11.5px] text-ink-muted">{{ auth.user?.email }}</span>
            </div>
          </button>
          <NuxtLink :to="localePath('/dashboard/mi-cuenta')"
          ...
          <NuxtLink :to="localePath('/dashboard/invitaciones')"
            class="mt-1 flex w-full items-center gap-2 rounded-[10px] px-2.5 py-2.5 text-left text-[13.5px] text-ink transition-colors hover:bg-bg-soft"
            @click="close">
            <DashboardIcon name="mail" :size="14" />
            <span>{{ t('dashboard.my_invitations.menu_entry') }}</span>
